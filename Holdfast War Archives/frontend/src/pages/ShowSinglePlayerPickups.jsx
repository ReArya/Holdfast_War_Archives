...
//         `http://localhost:5555/Pickups/public?page=${page}&limit=${pagination.limit}&search=${search}&sort=-Date`
//       );
...
//                             <XAxis 
//                               dataKey={(record) => new Date(record.Date).toLocaleDateString()} 
...
// Import API service instead of axios directly
import React, { useState, useEffect, useCallback } from 'react';
import debounce from 'lodash/debounce';
...
const PlayerStatsPage = () => {
  ...
  const fetchSuggestions = async (search) => {
    try {
      // Use API service instead of direct axios call
      const response = await api.get(`/Pickups/suggestions?search=${search}`);
      if (response.data && Array.isArray(response.data)) {
        setSuggestions(response.data);
      }
    } catch (err) {
